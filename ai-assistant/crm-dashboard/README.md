...
| POST | `/api/product-info/parse` | 输入 `raw_text`，返回产品拆分建议（中英文标签与数量/价格/用途启发式） |
...
Alibaba CSV 列顺序固定为：ID、姓名、公司、地址、电话、邮箱、产品原文、产品名称、香型要求、产品用途、产品数量、产品规格、目标价格、其他要求、备注。页面使用带 `href`/`download` 属性的直接下载链接，服务器同时返回 `Content-Disposition`；原文中的逗号、换行由 CSV 编码处理，Excel 可直接打开中文。
...
覆盖内容包括中文模板与旧版 `field/value` 表格解析、三源合并和板块归类、空 Alibaba 新建、地址与产品拆分字段持久化、中文/英文标签和数量/价格/用途启发式、索引/pipeline/档案同步、编辑时保留沟通记录、产品解析与 BOM CSV API、敏感日志脱敏，以及健康检查和客户列表 API；前端静态回归检查板块、拆分接口、CSV 和新增弹窗取消按钮。
