...
- `tests/test_server.py`：解析、合并/板块归类、空 Alibaba、新字段持久化、12 条混合批量样例顺序/对应/安全、纯数字标签/连续标签/缺失侧重读/API/CSV、复制控件、空位保留与 manual 静态断言、分页合同与移动端样式、旧邮件回归、HTTP、日志脱敏和错误码测试。
...
- **CRM-UI-001 已验证修复：** 原审计曾复现新增客户弹窗底部“取消”无效；已补充 `#add-cancel` → `closeModal(#add-dialog)`，最终 Browser 验收确认弹窗关闭且不提交，客户总数保持 77。
